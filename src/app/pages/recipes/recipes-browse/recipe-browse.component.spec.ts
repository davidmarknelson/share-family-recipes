...
function selectElements() {
	ingredientsInput = fixture.debugElement.query(By.css("[name=ingredient]"))
		.nativeElement;
	ingredientsSubmit = fixture.debugElement.query(By.css("[type=submit]"));
	ingredientsError = fixture.debugElement.query(
		By.css(".filters__ingredient-error")
	);
	ingredientsTag = fixture.debugElement.queryAll(By.css(".tag"));
	ingredientsTagDelete = fixture.debugElement.queryAll(
		By.css("[data-test=ingredient-tag-delete]")
	);
	clearIngredientsBtn = fixture.debugElement.query(
		By.css("[data-test=clear-ingredients]")
	);
	...
	closeErrorMsg = fixture.debugElement.query(
...
describe("RecipeBrowseComponent", () => {
	...
	describe("initiation", () => {
		...
		it("should show an error message when there are no recipes and close the the error message", () => {
			...
			expect(errorMsg.nativeElement.innerText).toContain(
				"There are no recipes with those ingredients."
			);
	...
	describe("ingredients", () => {
		...
		it("should show an error if the user tries to submit an empty ingredient", () => {
			...
			expect(ingredientsError.nativeElement.innerText).toContain(
				"You must add an ingredient."
			);
		});
		...
		it("should add an ingredient to the ingredients list", () => {
			...
			spyOn(searchesService, "ingredientsByNewest").and.callFake(() =>
				of(recipesObj)
			);
		...
		it("should get recipes by ingredients when changing sorting options", () => {
			...
			spyOn(component, "getIngredientData").and.callThrough();
			spyOn(component, "getSortingOptionForIngredients").and.callThrough();
			spyOn(searchesService, "ingredientsByOldest").and.callFake(() =>
				of(recipesObj)
			);
		...
		it("should get recipes by ingredients when changing amount options", () => {
			...
			expect(component.getIngredientData).toHaveBeenCalled();
			expect(component.getSortingOptionForIngredients).toHaveBeenCalled();
			expect(searchesService.ingredientsByNewest).toHaveBeenCalledWith(0, 18, [
				"egg",
			]);
		});
	});
