...
export class RecipeBrowseComponent implements OnInit, OnDestroy {
	...
	data: Object = {
		...
		byNamesAtoZ: (offset, limit) =>
			this.searchesService.byNamesAtoZ(offset, limit),
		byNamesZtoA: (offset, limit) =>
			this.searchesService.byNamesZtoA(offset, limit),
	};
	...
	getIngredientData(
		...
		return this.ingredientData[sortingOption](offset, limit, ingredients)
			.pipe(takeUntil(this.ngUnsubscribe))
			.subscribe(
				(res: RecipeCardInfo) => {
					this.errorMessage = "";
