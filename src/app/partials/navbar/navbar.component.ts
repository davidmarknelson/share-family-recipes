...
export class NavbarComponent implements OnInit {
			...
			.valueChanges.pipe(
				switchMap(val => {
					...
					if (this.isAutocompleteOptionSelected) {
						return of(
							(this.isAutocompleteOptionSelected = !this
	...
	onKeydown(key) {
		...
		if (
			...
			return this.searchBarForm
				.get("name")
				.setValue(this.autocompleteItems[this.highlightedIndex].name);
		}
	}
	...
	onSubmit() {
		this.router.navigateByUrl(
			`/recipes/${this.searchBarForm.get("name").value}`
		);
