...
describe('SavedRecipesService', () => {
  ...
  describe('savedRecipesAtoZ', () => {
    it('should return an array of saved recipes', () => {
      ...
      http.expectOne('http://localhost:3000/saved/a-z?offset=0&limit=9').flush(recipesObj);
      expect(response).toEqual(recipesObj);
      http.verify();
    });

    it('should return an error message on error', () => {
      const likeResponse = 'There was an error getting your list of saved recipes.';
      let errorResponse;

      savedRecipesService.savedRecipesAtoZ(0, 9).subscribe(res => {}, err => {
        errorResponse = err;
      });

      http.expectOne('http://localhost:3000/saved/a-z?offset=0&limit=9').flush({message: likeResponse}, {status: 500, statusText: 'Server Error'});
      expect(errorResponse.error.message).toEqual(likeResponse);
      http.verify();
    });
  });
  ...
  describe('savedRecipesZtoA', () => {
    it('should return an array of saved recipes', () => {
      ...
      http.expectOne('http://localhost:3000/saved/z-a?offset=0&limit=9').flush(recipesObj);
      expect(response).toEqual(recipesObj);
      http.verify();
    });
    ...
    it('should return an error message on error', () => {
      ...
      savedRecipesService.savedRecipesZtoA(0, 9).subscribe(res => {}, err => {
        errorResponse = err;
      });

      http.expectOne('http://localhost:3000/saved/z-a?offset=0&limit=9').flush({message: likeResponse}, {status: 500, statusText: 'Server Error'});
      expect(errorResponse.error.message).toEqual(likeResponse);
      http.verify();
    });
  });
